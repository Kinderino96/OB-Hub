...
export default async function Home() {
  ...
  const statuses = await Promise.all(
    dashboards.map(async (d) => {
      const token = cookieStore.get(getCookieName(d.id))?.value;
      ...
      <div style={{ maxWidth: 900, margin: "2.5rem auto", padding: "0 1.5rem" }}>
        ...
        <div style={{
          ...
          {dashboards.map((d) => {
            ...
            return (
              <Link key={d.id} href={unlocked ? `/dashboard/${d.id}` : `/login/${d.id}`} style={{ textDecoration: "none" }}>
                <div style={{
                  background: "#fff", border: "1px solid #e2e0d8",
                  borderRadius: 10, padding: "1.1rem 1.2rem",
                  cursor: "pointer", position: "relative",
                }}>
                  <div style={{ position: "absolute", top: 10, right: 10, fontSize: 14, opacity: 0.6 }}>
                    {unlocked ? "🔓" : "🔒"}
                  </div>
                  <div style={{
                    width: 40, height: 40, borderRadius: 8,
                    background: d.iconBg, display: "flex",
                  ...
                  <div style={{
                    ...
                    background: d.iconBg, color: d.iconColor, fontFamily: "monospace",
                  }}>
                    {d.category}
                  </div>
                </div>
